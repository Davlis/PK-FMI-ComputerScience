...
def easome(args):
    x1, x2 = args
    return cos(x1)*cos(x2)*pow(e, -pow(args[0]-25, 2)-pow(args[1]-25, 2))
...
class Population:
    ...
    eval_pop - populacja (dla wartości funkcji)
    """
    def __init__(self, func, sizeOfPopulation, amountOfVariables, amountOfIterations, pk, pm, a, b, dx):
        self.func = func
        self.sizeOfPopulation, self.amountOfVariables = sizeOfPopulation, amountOfVariables
    ...
    def gen_population(self):
        return np.random.randint(2, size=(self.sizeOfPopulation, self.amountOfVariables * self.amountOfBits))
    ...
    def cross(self, pop):
        for cur in range(self.sizeOfPopulation):
            if np.random.random_sample() <= self.probabilityOfCross:
                ...
                while True:
                    ano = np.random.randint(self.sizeOfPopulation)
                    if cur != ano:
                        break
...
Population(obj_func, 50, 2, 2000, 0.9, 0.02, -1.5, 1.5, 0.001)